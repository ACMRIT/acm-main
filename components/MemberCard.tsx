    ...
    return (
        <div className={`${isFaculty ? 'w-64' : 'w-48'}`}>
            <div className='flex flex-col justify-evenly bg-slate-100 items-center rounded-lg p-5 hover:bg-slate-300 hover:scale-105 transition duration-500 group h-full'>
                <div className={`${isFaculty ? 'w-32 h-32' : 'w-24 h-24'} relative rounded-full overflow-hidden shadow-lg transition-transform transform group-hover:scale-105 cursor-pointer`}>
                    <Image
                        src={member.image}
                        alt={`${member.name}`}
                        fill
